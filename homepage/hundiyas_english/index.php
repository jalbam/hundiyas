...
<html>
    <head>
        <title>Hundiyas &copy; (by Joan Alba Maldonado)</title>
        <!-- (c) Hundiyas - Program by Joan Alba Maldonado (granvino@granvino.com). Prohibited to publish, reproduce or modify without maintain author's name. -->
        ...
        <!--
                ...
                function escoger_barco(numero)
                {
                    ...
                    for (x = 1; x <= 5; x++)
                    {
                        ...
                        if (x == numero || barcos_puestos[x-1])
                        {
                            ...
                            document.getElementById("barco_" + x + "_invisible").title = "Leave ship";
                            document.getElementById("barco_" + x + "_img_invisible").title = "Leave ship";
                        }
                    }
                ...
                {
                    //Tener en cuenta el tamaño del barco arrastrandose, si esta en vertical o en horizontal y los barcos que crucen:
                    //Se define el incremento del bucle segun este el barco en vertical o en horizontal:
                    var incremento_bucle = (posicion_barco == "horizontal") ? 1 : tablero_ancho;
                    //Se define el tope del bucle segun este el barco en vertical o en horizontal:
                ...
                function mostrar_ocultar_opciones()
                {
                    ...
                    if (document.getElementById("menu_opciones").style.visibility == "visible") { document.getElementById("menu_opciones").style.visibility = "hidden"; document.getElementById("menu_opciones_sombra").style.visibility = "hidden"; document.getElementById("menu").title = "Open options"; }
                    ...
                    else
                    {
                        ...
                        document.getElementById("menu_opciones").style.visibility = "visible"; document.getElementById("menu_opciones_sombra").style.visibility = "visible"; document.getElementById("menu").title = "Close options";
                    }
                }
                ...
                function aplicar_opciones()
                {
                    ...
                    if (errores != "")
                    {
                        ...
                        else
                        {
                                        ...
                                        mostrar_alerta("Options cannot be applied because:<br>" + errores.replace(/[\n]/g, "<br>"), false);
                        }
                        return;
                    }
                    //...pero si todo esta correcto, se pide confirmacion y si acepta se procede a aplicar las opciones:
                    else if (!alerts_activados || alerts_activados && confirm("The selected options are going to be applied. Present game will be lost."))
                    {
...
//alert("barco "+ numero_barco+" BORRADO de "+celda_a_probar+" en posicion "+posicion_barco);
                ...
                function comenzar_batalla()
                {
                    ...
                    document.getElementById("barcos").innerHTML = "&nbsp; Sunk ships:";
                    ...
                    {
                        if (alerts_activados) { alert("It was not possible to be placed the ships of the computer. Try to extend the board and try again."); iniciar_juego(true); }
                        else { mostrar_alerta("It was not possible to be placed the ships of the computer. Try to extend the board and try again.", true); }
                    }
...
//alert("se ke barko buskar i es el " + barco_alcanzado + "("+cruz_barco[barco_alcanzado]["vertical"][0]+", "+cruz_barco[barco_alcanzado]["vertical"][1]+", "+cruz_barco[barco_alcanzado]["horizontal"][0]+", "+cruz_barco[barco_alcanzado]["horizontal"][1]+")");
                        ...
                        else if (contenido_celda != "B" && contenido_celda != "X")
                        {
                            ...
                            if (barcos[oponente][contenido_celda-1] == 0)
                            {
                                ...
                                if (oponente == "usuario")
                                {
                                    ...
                                    mostrar_mensaje("Your ship " + contenido_celda + " has been touched and sunk.");
                                    if (alerts_activados) { alert("Your ship " + contenido_celda + " has been touched and sunk."); mostrar_mensaje(""); }
                                }
                                ...
                                else
                                {
                                    se_ha_hundido_alguno = true;
                                    document.getElementById("bomba").style.visibility = "hidden";
                                    mostrar_mensaje("Ship " + contenido_celda + " of the computer has been touched and sunk.");
                                    document.getElementById("barcos").innerHTML += " [" + contenido_celda + "]";
                                    if (alerts_activados) { alert("Ship " + contenido_celda + " of the computer has been touched and sunk."); mostrar_mensaje(""); }
                                }
                ...
                function calcular_fin_de_juego(ultimo_atacante)
                {
                    ...
                    if (!quedan_barcos)
                    {
                        ...
                        if (oponente == "usuario")
                        {
                            mostrar_mensaje("Game over. You lost.");
                            document.getElementById("reloj").style.visibility = "hidden";
                            if (alerts_activados) { alert("Game over. You lost."); }
                            else { mostrar_alerta("Game over. You lost.", true); }
                        }
                        else
                        {
                            mostrar_mensaje("Congratulations, you won.");
                            document.getElementById("reloj").style.visibility = "hidden";
                            if (alerts_activados) { alert("Congratulations, you won."); }
                            else { mostrar_alerta("Congratulations, you won.", true); }
                        }
                        if (alerts_activados)
                        {
                            //Se muestra el mensaje de espera:
                ...
                function iniciar_juego_primera_vez()
                {
                    ...
                    //Se muestra el mensaje de espera:
                    setTimeout('mostrar_mensaje("Loading...");', 10)
                ...
                function iniciar_juego(mismos_tableros)
                {
                    ...
                    //Si se ha seteado adaptar las celdas segun el tamaño del mapa, se adapta:
                    if (adaptar_celdas)
                    {
    ...
    <body onLoad="pagina_cargada = false; document.getElementById('reloj').style.visibility = 'hidden'; setTimeout('iniciar_juego_primera_vez(); pagina_cargada = true;', 10);" onMouseUp="campo_seleccionable = false;" onClick="campo_seleccionable = false;" onMouseMove="if (pagina_cargada) { arrastrar_opciones(event); arrastrar_alerta(event); arrastrar_barco(event, barco_arrastrandose); arrastrar_bomba(event); arrastrar_reloj(event); }" onContextMenu="return rotar_barco();" onKeyPress="javascript:tecla_pulsada(event, 'onkeypress');" onKeyDown="javascript:tecla_pulsada(event, 'onkeydown');" bgcolor="#9999aa" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
        <!-- Menu: -->
        <div id="juego_nuevo" style="left:8px; top:10px; width:110px; height:20px; position:absolute; border:0px; padding:0px; background:transparent; color:#000000; text-align:center; line-height:20px; text-decoration:none; font-family:arial; font-size:12px; cursor:pointer; cursor:hand; -moz-user-select:none; z-index:2;" title="New game" onMouseOver="if (!juego_bloqueado) { this.style.fontWeight = 'bold'; }" onMouseOut="this.style.fontWeight = 'normal';" onClick="if (!juego_bloqueado && se_ha_comenzado && (!alerts_activados || alerts_activados && confirm('Press ok to start a new game. Present game will be lost.'))) { mostrar_mensaje('Loading...'); setTimeout('iniciar_juego(true);', 10); }" onSelectStart="return false;">
            New game
        </div>
                ...
                <form style="display:inline;" onSubmit="aplicar_opciones(); return false;" align="center">
                   <center>
                       <fieldset style="width:310px; border:1px #000055 solid; cursor:crosshair;">
                           <legend style="width:310px; color:#0000aa; font-size:20px; font-weight:bold; text-align:center; -moz-user-select:none;" onSelectStart="return false;" title="Options menu">Options</legend>
                           <br>
                           <label for="tablero_ancho" style="line-height:12px; font-size:12px; cursor:pointer; cursor:hand;" accesskey="t" title="Board width (squares number)"><b style="-moz-user-select:none;" onSelectStart="return false;">&nbsp; Board wid<u>t</u>h:</b> <input type="text" name="tablero_ancho" id="tablero_ancho" onMouseDown="campo_seleccionable = true;" onMouseUp="campo_seleccionable = false;" size="4" maxlength="3" style="height:22px; color:#0000aa; background:#ffffdd; font-family:courier; line-height:12px; font-size:12px; font-weight:bold;" accesskey="t"></label>
                           <br>
                           <br>
                           <label for="tablero_alto" style="line-height:12px; font-size:12px; cursor:pointer; cursor:hand;" accesskey="e" title="Board height (squares number)"><b style="-moz-user-select:none;" onSelectStart="return false;">&nbsp; Board h<u>e</u>ight:</b> <input type="text" name="tablero_alto" id="tablero_alto" onMouseDown="campo_seleccionable = true;" onMouseUp="campo_seleccionable = false;" size="4" maxlength="3" style="height:22px; color:#0000aa; background:#ffffdd; font-family:courier; line-height:12px; font-size:12px; font-weight:bold;" accesskey="e"></label>
                           <br>
                           <br>
                           <label for="permitir_bombardear_lo_bombardeado" style="line-height:12px; font-size:12px; cursor:pointer; cursor:hand;" accesskey="b" title="Allow to bomb again a sqare"><input type="checkbox" name="permitir_bombardear_lo_bombardeado" id="permitir_bombardear_lo_bombardeado" onMouseDown="campo_seleccionable = true;" onMouseUp="campo_seleccionable = false;" accesskey="b" style="cursor:pointer; cursor:hand;"><b style="-moz-user-select:none;" onSelectStart="return false;"> Can <u>b</u>omb squares infinitely</b></label>
                           <br>
                           <br>
                           <label for="permitir_poner_barcos_adyacentes" style="line-height:12px; font-size:12px; cursor:pointer; cursor:hand;" accesskey="j" title="Allow to put ships in contiguous squares"><input type="checkbox" name="permitir_poner_barcos_adyacentes" id="permitir_poner_barcos_adyacentes" onMouseDown="campo_seleccionable = true;" onMouseUp="campo_seleccionable = false;" accesskey="j" style="cursor:pointer; cursor:hand;"><b style="-moz-user-select:none;" onSelectStart="return false;"> Able to put ships ad<u>j</u>acent</b></label>
                           <br>
                           <br>
                           <label for="marcar_bombardeado" style="line-height:12px; font-size:12px; cursor:pointer; cursor:hand;" accesskey="m" title="Mark bombed squares"><input type="checkbox" name="marcar_bombardeado" id="marcar_bombardeado" onMouseDown="campo_seleccionable = true;" onMouseUp="campo_seleccionable = false;" accesskey="m" style="cursor:pointer; cursor:hand;"><b style="-moz-user-select:none;" onSelectStart="return false;"> <u>M</u>ark bombed squares</b></label>
                           <br>
                           <br>
                           <label for="primer_jugador" style="line-height:12px; font-size:12px; cursor:pointer; cursor:hand;" accesskey="i" title="What user bomb first?">
                           ...
                           <br>
                           <br>
                           <label for="dificultad" style="line-height:12px; font-size:12px; cursor:pointer; cursor:hand;" accesskey="d" title="Difficulty level">
                           <b style="-moz-user-select:none;" onSelectStart="return false;">&nbsp; <u>D</u>ifficulty:</b>
                           ...
                           <br>
                           <br>
                           <center><input type="submit" value="Apply" name="boton_aplicar" style="height:24px; color:#aa0000; font-weight:bold; text-align:center; line-height:12px; font-size:12px; font-family:arial; cursor:pointer; cursor:hand; -moz-user-select:none;" accesskey="a" onSelectStart="return false;" title="Apply options"></center>
                           <br>
                       </fieldset>
                    </center>
                </form>
        ...
        <div id="informacion" style="left:10px; top:490px; height:0px; position:absolute; border:0px; padding:0px; background:transparent; color:#333333; text-align:left; line-height:20px; text-decoration:none; font-family:verdana; font-size:10px; z-index:3;">
            &copy; <b>Hundiyas</b> 0.29a by <i>Joan Alba Maldonado</i> (<a href="mailto:granvino@granvino.com">granvino@granvino.com</a>) &nbsp;<sup>(DHTML 100%)</sup>
